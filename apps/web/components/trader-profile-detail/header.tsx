  ...
  return (
    <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div className="flex min-w-0 items-start gap-4">
        <Link
          href="/leaderboard"
          className="focus-ring mt-4 grid size-10 shrink-0 place-items-center rounded-full text-zinc-500 transition hover:bg-white hover:text-zinc-950 dark:hover:bg-zinc-900 dark:hover:text-zinc-50"
          onFocus={prefetchLeaderboard}
          onMouseEnter={prefetchLeaderboard}
        >
          <ArrowLeft size={22} />
        </Link>
        <div className={`mt-2 grid size-16 shrink-0 place-items-center rounded-full bg-gradient-to-br ${visual.tone} font-mono text-xl font-bold text-white shadow-sm`}>
          {visual.initials}
        </div>
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="truncate text-2xl font-semibold tracking-tight text-zinc-950 sm:text-3xl dark:text-zinc-50">{localizedName}</h1>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-600 dark:text-zinc-300">{localizedDescription}</p>
        </div>
      </div>
      <div className="grid w-full grid-cols-3 gap-2 rounded-2xl bg-white p-2 ring-1 ring-zinc-200 dark:bg-zinc-950 dark:ring-zinc-800 lg:w-auto">
...
function MiniHeroStat({ label, value, tone }: { label: string; value: string; tone?: "good" | "bad" }) {
  return (
    <div className="min-w-0 rounded-xl bg-zinc-50 px-3 py-3 sm:px-4 dark:bg-zinc-900">
