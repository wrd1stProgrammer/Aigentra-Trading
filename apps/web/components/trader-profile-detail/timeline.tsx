...
export function TimelineRow({ item, index, onClick }: { item: TimelineItem; index: number; onClick?: () => void }) {
  ...
  const content = (
    <>
      <div className="relative z-[1] mt-1 grid size-7 place-items-center rounded-full bg-white dark:bg-zinc-950">
      ...
      <div className="min-w-0">
        ...
        <div className="grid gap-4 sm:grid-cols-[48px_minmax(0,1fr)_24px] sm:items-center">
          <div className="grid size-12 place-items-center rounded-lg bg-zinc-100 font-mono text-xs font-bold text-zinc-700 dark:bg-zinc-900 dark:text-zinc-200">
            {item.iconLabel}
          </div>
          <div className="min-w-0">
          ...
          <CaretRight className="hidden text-zinc-400 sm:block" size={22} />
        </div>
      </div>
    </>
  );

  if (!onClick) return <div className="grid grid-cols-[28px_minmax(0,1fr)] gap-5">{content}</div>;
  return (
    <button type="button" onClick={onClick} className="grid w-full grid-cols-[28px_minmax(0,1fr)] gap-5 rounded-xl text-left transition hover:bg-zinc-50 dark:hover:bg-zinc-900/70">
      {content}
    </button>
  );
}
