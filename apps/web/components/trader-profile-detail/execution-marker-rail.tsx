          ...
          {cycles.length ? (
            cycles.map((cycle) => (
              <ExecutionMarkerChip
                key={cycle.id}
                cycle={cycle}
                selected={cycle.markers.some((marker) => marker.id === selectedId)}
                onSelect={onSelect}
                t={t}
              />
            ))
...
function ExecutionMarkerChip({
  ...
  return (
    <button
      ...
      onClick={() => onSelect(cycle.representativeId)}
      aria-pressed={selected}
      title={`${cycle.titleLabel} · ${cycle.priceSummaryLabel}`}
    >
      <span className={`grid size-7 shrink-0 place-items-center rounded-md ${iconToneClass(cycle)}`}>
