        ...
        {telegramReviewSections.map((section) => {
          ...
          return (
            <button
              ...
              className={`focus-ring flex flex-col items-start justify-between rounded-lg border p-2.5 sm:p-3.5 text-left transition duration-200 ${
                isChecked
            ...
            >
              <div className="flex items-center gap-2">
                <span
                  className={`inline-flex size-3.5 sm:size-4 items-center justify-center rounded border ${
                    isChecked ? "border-sky-500 bg-sky-500 text-white" : "border-zinc-300 dark:border-white/10"
                  }`}
                >
                  {isChecked && <Check size={8} weight="bold" />}
                </span>
                <span className="text-[11px] sm:text-xs font-bold leading-tight">{labels[section]}</span>
              </div>
              <p className="hidden sm:block mt-2 text-[10px] leading-relaxed text-zinc-400 break-keep dark:text-zinc-500">
