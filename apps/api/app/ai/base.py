...
def position_management_review_prompt(payload: PositionManagementPayload) -> str:
    ...
    return (
        ...
        "structuredReview is the primary user-facing explanation. It must be an object with verdict, headline, action, keyReasons, risks, watchConditions, managerNote. "
        "Write structuredReview as a position management briefing for a normal user who wants to understand the current position and what to watch next. "
        ...
        "headline is one plain sentence that says whether the thesis is working, weakening, protected, or invalidated. "
        "action is one concrete next move in one sentence only and never a list: hold, cancel, reduce, close, move stop, or wait for a named trigger. "
        "keyReasons has up to 2 short bullets explaining why the action follows from price versus entry/stop/target, recent candles, momentum/volume, and recent reviews/events. "
        "risks has up to 1 short bullet naming what can go wrong next, and watchConditions has up to 2 exact price/timeframe/indicator triggers that would change the decision. "
        "managerNote is optional one concise desk note about how to manage the next review window. Do not write raw JSON/Python list syntax inside any string. "
