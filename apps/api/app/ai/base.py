...
def entry_approval_prompt(payload: TradeReviewPayload) -> str:
    ...
    return (
        ...
        "A clean setup may keep leverage within leveragePlan.maxLeverage, but AI confidence is not permission to expand approved risk or exceed available cash. "
        ...
        "APPROVED ENTRY REASON RULE: for approved decisions, write the user-facing reason as an entry thesis first and a risk check second. "
        "For approved entries, structure user-facing copy in this order: entry decision, trader-specific interpretation, then risk boundary. "
        "headline must be the plain answer to why this trader entered: name the trader concept or setup family, the entry zone, and the market trigger in one sentence. "
        "approvalReason must answer the entry reason before risk controls: what market trigger appeared, why this price zone is the entry, and what confirmation makes waiting less attractive. "
        "keyReasons[0] must name the market trigger that made this entry worth taking now and be the trader-specific interpretation of the trigger, not a generic indicator list; keyReasons[1] may explain stop, target, RR, or residual risk. "
        ...
        "the concrete entry trigger and trader thesis before mentioning entry/stop/target geometry, fee-aware RR, risk adjustment, or the main residual risk. "
...
def position_management_review_prompt(payload: PositionManagementPayload) -> str:
    ...
    shock_instruction = (
        ...
        "Never widen stops; never exceed leverage, available cash and approved risk constraints. For this event, set nextReviewInSeconds to 120 unless the position/order is closed or cancelled. "
    ...
    return (
        ...
        "Never widen a stop; never exceed leverage, available cash and approved risk constraints. "
        ...
        "CURRENT POSITION DECISION TEMPLATE: every open-position structuredReview must answer four user questions in simple language: "
        "what is happening now, why this action is better than close/stop move/take profit, what changed since the previous review, and what exact next trigger changes the call. "
        "If recentReviewMemory repeats the same title, action, or reason, choose a different useful angle from current price movement, PnL/R change, stop distance, target distance, or the trader's own management anchor. "
        "Do not write a review that could apply to any trader; name the relevant trader-specific anchor and connect it to the current stop/target box. "
        "A user should never need to infer the entry reason or management reason from buzzwords; spell out the practical reason in one plain sentence. "
...
def league_sentiment_prompt(payload: LeagueSentimentPayload) -> str:
    ...
    return (
        ...
        "Write brief as a BTC market briefing, not as a transaction log. "
        "brief must have exactly conclusion, reason, and watch. These three fields should read as a compact three-line market briefing together. "
        ...
        "Across the three brief fields, write roughly three to five compact sentences total when the payload supports it; do not collapse the opinion into one bland headline. "
        "Every brief must contain a market thesis: BTC regime, trader read, and the practical implication for this league context. "
        "brief.conclusion should be two punchy sentences when data allows: name the current BTC regime and the directional pressure in the same field, then say whether this is chase, retest, compression, defense, or risk-off. "
        ...
        "brief.reason must synthesize active exposure, pending orders, fresh AI reviews, and recent outcomes instead of reading any one table aloud. "
        "Use this field to explain whether trader behavior confirms, fades, or hesitates around the BTC regime. "
        ...
        "brief.watch must give one concrete level, zone, timeframe close, or exposure change and what that would mean for the read. "
        "Forbid limp summaries such as 'not one-sided', 'monitor closely', or only restating LONG/SHORT counts. "
        "If data is mixed, say what the conflict means: e.g. confirmation-first, failed retest risk, crowded chase risk, defensive pause, or compression. "
        "The user should learn the BTC situation through the traders, not just learn what the traders recently did. "
        "Do not recap recent trade events as the briefing; recent take-profit/stop-loss events are supporting context, not the main story. "
        "The three brief fields together must answer: what BTC is doing now, how traders are positioned, and what judgment follows for this league context. "
