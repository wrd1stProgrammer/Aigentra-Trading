...
class DonchianBreakout(TraderStrategy):
    ...
    def evaluate(self, snapshot: Dict[str, Any]) -> TradeCandidate:
        ...
        g["volumeZ15m"] = completed_volume_z
        g["candleBody"] = signal_body
        volume_confirmed = completed_volume_z >= 0.35
        oi_confirmed = float(g["oi30m"]) >= 0.12
        taker_share = float(g["takerBuyShare"])
        long_taker_confirmed = taker_share >= 0.55
        short_taker_confirmed = taker_share <= 0.45
        high_break = signal_close > upper and g["trend4h"] != "bearish"
        low_break = signal_close < lower and g["trend4h"] != "bullish"
        if high_break:
            side, setup = "LONG", "DONCHIAN_RANGE_EXPANSION_LONG"
            taker_confirmed = long_taker_confirmed
            broken_boundary = upper
        elif low_break:
            side, setup = "SHORT", "DONCHIAN_RANGE_EXPANSION_SHORT"
            taker_confirmed = short_taker_confirmed
            broken_boundary = lower
        else:
            g["donchianParticipationCount"] = 0
            return _rejection(self.profile, "A completed 15m candle has not closed through the frozen Donchian boundary.", 0, g, "donchian_no_breakout")
        ...
        g["donchianParticipationCount"] = participation_count
        ...
        if participation_count < 2:
            return _rejection(self.profile, "Donchian breakout needs at least two directional participation confirmations.", score, g, "donchian_participation_weak")

        price = signal_close
        atr_buffer = float(g["atr1h"]) * 0.20
        boundary_stop_distance = (
            price - (broken_boundary - atr_buffer)
            if side == "LONG"
            else (broken_boundary + atr_buffer) - price
        )
        risk_distance = max(float(g["atr1h"]) * 1.20, float(g["atr4h"]) * 0.34, price * 0.006, boundary_stop_distance)
